...
namespace tm_gen
{
  ...
  TEST_F(B4Test, NoFitExtra)
  {
    ...
    ASSERT_TRUE(HasPath(*routing, "[A->B]", 0.833));
    ASSERT_TRUE(HasPath(*routing, "[A->D, D->C, C->B]", 0.166));
    ASSERT_TRUE(HasPath(*routing, "[D->C]", 1));
  }

  class MinMaxPathTest : public TestBase
  {
  protected:
    MinMaxPathTest() : tm_(&graph_) {}
  ...
  // Fits on the shortest path.
  TEST_F(MinMaxPathTest, SinglePath)
  {
  ...
  // Does not fit on the shortest path.
  TEST_F(MinMaxPathTest, SinglePathNoFitLowerCapacity)
  {
    MinMaxPathBasedOptimizer minmaxpath_optimizer(path_provider_.get(), 1.0, true, 10);
    AddAggregate("A", "B", kDefaultLinkspeed * 1.2);
    auto routing = minmaxpath_optimizer.Optimize(tm_);
    ...
    /* for (const auto &aggregate_and_routes : routing->routes())
    {
      ...
      for (const auto &route_and_fraction : routes)
      {
        ...
        LOG(ERROR) << "PATH : " << route_and_fraction.first->ToStringNoPorts(graph_) << " fraction : " << fraction_in_result;
      }
    } */
  ...
  // A couple of aggregates that do not fit on the shortest path
  TEST_F(MinMaxPathTest, MultiSinglePathNoFit)
  {
    MinMaxPathBasedOptimizer minmaxpath_optimizer(path_provider_.get(), 1.0, true, 10);
    AddAggregate("A", "B", kDefaultLinkspeed * 0.6);
    AddAggregate("A", "C", kDefaultLinkspeed * 0.6);
  ...
  // A couple of aggregates, one does not fit.
  TEST_F(MinMaxPathTest, MultiSinglePathOneNoFit)
  {
    MinMaxPathBasedOptimizer minmaxpath_optimizer(path_provider_.get(), 1.0, true, 10);
    AddAggregate("A", "B", kDefaultLinkspeed * 0.3);
    AddAggregate("A", "C", kDefaultLinkspeed * 0.9);
  ...
  // Traffic does not fit.
  TEST_F(MinMaxPathTest, NoFitExtra)
  {
    MinMaxPathBasedOptimizer minmaxpath_optimizer(path_provider_.get(), 1.0, true, 10);
    AddAggregate("A", "B", kDefaultLinkspeed * 3);
    AddAggregate("D", "C", kDefaultLinkspeed * 2);
